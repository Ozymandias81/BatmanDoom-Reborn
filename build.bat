...
TITLE Batman Doom: Reborn Compiler - Thanks Tiger!
...
EXPLORER https://github.com/Ozymandias81/BatmanDoom-Reborn
ECHO.
GOTO :EOF
...
IF %featuresGit% NEQ True (
    ...
    IF "%projectName%" NEQ "batrebrn" SET "projectName=batrebrn"
    GOTO :EOF
)
...
REM Avoid redundancy
IF "%projectName%" NEQ "batrebrn-%GitCommitHash%" SET "projectName=batrebrn-%GitCommitHash%"
GOTO :EOF
